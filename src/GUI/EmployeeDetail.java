...
public class EmployeeDetail extends JDialog {
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (btnNewButton_1.getText().equals("Chỉnh sửa")) {
					...
					nv.setTrinhDo(new TRINHDO(nv.getTrinhDo().getMaTrinhDo(), cbbHocVan.getSelectedItem().toString(),
							cbbChuyenMon.getSelectedItem().toString(), listTextField.get(7).getText()));
	...
	public boolean checkInformation() {
		...
		if (!checkSalary(listTextField.get(9).getText())) {
			JOptionPane.showMessageDialog(this, "Mức lương không hợp lệ (lương dưới 4 củ và phải đúng định dạng)",
